...
use App\Models\TransactionModel; // Added to sync Lost/Damaged status
...
class Collections extends BaseController
{
    ...
    public function update()
    {
        ...
        // Upload new Soft Copy if provided
        ...
        if ($collectionModel->update($id, $data)) {
            ...
            // --- NEW FIX: AUTO-UPDATE TRANSACTIONS IF BOOK IS REPORTED LOST OR DAMAGED ---
            if (in_array($data['status'], ['LOST', 'DAMAGED'])) {
                $transModel = new TransactionModel();
                
                // Find any active transactions for this specific book
                $activeTrans = $transModel->where('collection_id', $id)
                                          ->whereIn('status', ['Borrowed', 'Renewing', 'Approved', 'Pending'])
                                          ->findAll();
                
                // Convert 'LOST' to 'Lost' for the transaction table formatting
