                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php elseif($row['status'] == 'Renewing'): ?>
                                            <span class="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 px-2 py-1">Renewal Requested</span>
                                    ...
                                    <td class="text-center pe-4">
                                        <?php if($row['status'] == 'Borrowed'): ?>
                                            <button class="btn btn-sm btn-outline-primary fw-bold" onclick="openRenewModal('<?= $row['id'] ?>', '<?= esc(addslashes($row['collection_title'])) ?>')">
                                                Renew
                                            </button>
                                        <?php elseif($row['status'] == 'Renewing'): ?>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <form action="<?= base_url('borrower/renew/submit') ?>" method="POST" class="modal-content border-0 shadow">
            ...
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title fw-bold">Request Renewal</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4 text-center">
                <i class="bi bi-arrow-repeat text-primary fs-1 mb-3"></i>
                <p class="mb-1">Submit renewal request for:</p>
                <h6 class="fw-bold" id="renew_title"></h6>
                <p class="small text-muted mt-3">Admin will review and set a new return date for you.</p>
            </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-muted small">
                        <?= ($row['status'] == 'Returned') ? date('M d, Y', strtotime($row['updated_at'])) : '---' ?>
                    </td>
                    <td>
                        <?php 
                ...
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="4" class="text-center py-5">
                            <i class="bi bi-archive text-muted fs-2 d-block mb-2 opacity-25"></i>
                            <span class="text-muted">No past records found in your history.</span>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
