            ...
            <?php if(!empty($items)): ?>
                <?php foreach($items as $item): ?>
                    <?php 
                        ...
                        // Dynamically format the item status for badging and logic checks
                        $itemStatus = strtoupper($item['status']);
                        $statusBadgeClass = 'bg-success bg-opacity-10 text-success border-success'; // Default Available
                        
                        if ($itemStatus === 'BORROWED') {
                            $statusBadgeClass = 'bg-warning bg-opacity-10 text-dark border-warning';
                        } elseif (in_array($itemStatus, ['LOST', 'DAMAGED'])) {
                        ...
                        <div class="card-body p-4 ms-2">
                            <div class="row align-items-center">
                                <div class="col-md-9 d-flex align-items-start">
                                    ...
                                    <div>
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            <span class="badge border <?= $statusBadgeClass ?> border-opacity-25"><?= esc($item['status']) ?></span>
                                        </div>
                                ...
                                <div class="col-md-3 text-md-end mt-3 mt-md-0">
                                    ...
                                    <?php elseif ($itemStatus === 'LOST' || $itemStatus === 'DAMAGED'): ?>
                                        <button class="btn btn-danger w-100 btn-sm disabled opacity-75">
                                            <i class="bi bi-exclamation-circle-fill"></i> <?= ucfirst(strtolower($itemStatus)) ?>
                                        </button>

                                    <?php elseif ($itemStatus !== 'AVAILABLE'): ?>
                                        <button class="btn btn-light w-100 btn-sm disabled text-muted border">
...
<script>
    function openRequestModal(title, id) {
        ...
        // Explicitly trigger the modal using Bootstrap's JS API
