        ...
        <div class="col-lg-9">
                        ...
                        <div class="card-body p-4 ms-2">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-3 text-md-end mt-3 mt-md-0">
                                    <?php 
                                        // Find if the current user has a transaction for this book
                                        $userTransStatus = $userTransactions[$item['id']] ?? null;
                                    ?>

                                    <?php if ($userTransStatus): ?>
                                        <?php if ($userTransStatus === 'Pending'): ?>
                                            <button class="btn btn-secondary w-100 btn-sm disabled">
                                                <i class="bi bi-clock"></i> Already Requested
                                            </button>
                                        <?php elseif ($userTransStatus === 'Approved'): ?>
                                            <button class="btn btn-success w-100 btn-sm disabled">
                                                <i class="bi bi-box-seam"></i> Ready for Pickup
                                            </button>
                                        <?php elseif (in_array($userTransStatus, ['Borrowed', 'Renewing'])): ?>
                                            <button class="btn btn-info text-white w-100 btn-sm disabled">
                                                <i class="bi bi-person-check-fill"></i> On Hand
                                            </button>
                                        <?php endif; ?>

                                    <?php elseif ($itemStatus === 'BORROWED'): ?>
                                        <button class="btn btn-light w-100 btn-sm disabled text-muted border">
                                            <i class="bi bi-slash-circle"></i> Unavailable
                                        </button>

                                    <?php elseif ($itemStatus === 'DAMAGED'): ?>
                                        <button class="btn btn-outline-danger w-100 btn-sm fw-bold" 
                                            onclick="openRequestModal('<?= esc(addslashes($item['title'])) ?>', '<?= $item['id'] ?>', 'DAMAGED')">
                                            <i class="bi bi-exclamation-triangle"></i> Request (Damaged)
                                        </button>

                                    <?php elseif ($itemStatus === 'LOST' || $itemStatus !== 'AVAILABLE'): ?>
                                        <button class="btn btn-light w-100 btn-sm disabled text-muted border">
                                            <i class="bi bi-slash-circle"></i> Unavailable
                                        </button>

                                    <?php else: ?>
                                        <button class="btn btn-outline-dark w-100 btn-sm fw-bold" 
                                            onclick="openRequestModal('<?= esc(addslashes($item['title'])) ?>', '<?= $item['id'] ?>', 'AVAILABLE')">
                                            Request Access
                                        </button>
                                    <?php endif; ?>
                                </div>
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <p class="mt-3">No resources found matching your search.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <form action="/borrower/request/submit" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    <div id="damageWarningBox" class="alert alert-danger d-none small mb-4">
                        <i class="bi bi-exclamation-triangle-fill me-2"></i>
                        <strong>Notice:</strong> This item is marked as <b>Damaged</b>. Proceeding will request the damaged copy.
                        <div class="form-check mt-2">
                            <input class="form-check-input shadow-sm" type="checkbox" id="damageAcknowledge">
                            <label class="form-check-label fw-bold" for="damageAcknowledge">
                                I acknowledge that this resource is damaged.
                            </label>
                        </div>
                    </div>
                ...
                <div class="modal-footer bg-light">
                    ...
                    <button type="submit" id="submitRequestBtn" class="btn btn-primary shadow-sm fw-semibold" style="background-color: #1e3a8a; border-color: #1e3a8a;">
...
<script>
    function openRequestModal(title, id, status) {
        // Set basic values
        document.getElementById('modalBookTitle').value = title;
        document.getElementById('modalBookId').value = id;
        
        // Handle logic for Damaged warning check
        const warningBox = document.getElementById('damageWarningBox');
        const ackCheckbox = document.getElementById('damageAcknowledge');
        const submitBtn = document.getElementById('submitRequestBtn');

        if (status === 'DAMAGED') {
            document.getElementById('modalBookTitle').value = title + " [DAMAGED COPY]";
            warningBox.classList.remove('d-none');
            ackCheckbox.checked = false; // Reset checkbox
            ackCheckbox.required = true; // Make it required
            submitBtn.disabled = true;   // Lock submit button initially

            // Unlock submit only when checked
