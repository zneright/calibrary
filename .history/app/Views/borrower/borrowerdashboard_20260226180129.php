<?= $this->extend('layouts/borrower') ?>
    ...
    <div class="page-header text-center px-3 py-5 mb-4 text-white rounded-3 shadow-sm" style="background: linear-gradient(135deg, #1e3a8a 0%, #0f172a 100%);">
        <div class="container">
            <?php 
                ...
                $avatarPath = (!empty($currentUser['avatar'])) 
                    ...
                    : base_url('assets/images/default-avatar.png'); // Change this if you have a default image path
            ?>
            
            <div class="mb-3">
                <?php if(!empty($currentUser['avatar'])): ?>
                    <img src="<?= esc($avatarPath) ?>" alt="Profile" class="rounded-circle shadow-lg border border-3 border-white" style="width: 90px; height: 90px; object-fit: cover; background-color: #fff;">
                <?php else: ?>
                    <div class="rounded-circle shadow-lg border border-3 border-white d-inline-flex justify-content-center align-items-center" style="width: 90px; height: 90px; background-color: #e2e8f0; color: #1e3a8a;">
                                ...
                                <tr>
                                    <td class="ps-4 py-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <h6 class="mb-0 fw-semibold small text-truncate" style="max-width: 250px;">
                                                    <?= esc($row['collection_title']) ?>
                                                </h6>
                                                <small class="text-muted"><?= esc($row['class'] ?? 'General') ?></small>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="small">
                                    ...
                                    <td>
                                        <?php 
                                            ...
                                            // Logic to keep "Renewing" items labeled as On Hand but distinct
                                            if ($status == 'Borrowed' || $status == 'Renewing') { 
                                                $status = ($status == 'Renewing') ? 'Extension Pending' : 'On Hand'; 
                                                $badgeClass = (strtotime($row['due_date'] ?? '') < strtotime($today)) ? 'bg-danger' : 'bg-info text-white';
                                            }
                                        ?>
                                    ...
                                    <td class="text-end pe-4">
                                        <a href="/borrower/my-books" class="btn btn-sm btn-light border small px-3 shadow-sm">Manage</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="4" class="text-center py-5 text-muted small">No active transactions at the moment.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
                </div>
