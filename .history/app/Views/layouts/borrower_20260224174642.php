...
<script>
    // Wait for the dropdown to start showing
    ...
    if (notifDropdown) {
        notifDropdown.addEventListener('show.bs.dropdown', function () {
            ...
            if (badge && badge.style.display !== 'none') {
                // 1. Hide badge immediately for better UX
                badge.style.display = 'none';

                // 2. Update database status to 'read' via AJAX
                fetch('<?= base_url('borrower/markNotificationsRead') ?>', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Content-Type': 'application/x-www-form-urlencoded',
                        // CodeIgniter 4 looks for this header for CSRF
                ...
                .catch(error => {
                    ...
                    // Optional: show badge again if it failed
