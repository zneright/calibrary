...
<head>
    ...
    <style>
        body { background-color: #f8f9fa; font-family: 'Segoe UI', system-ui, sans-serif; display: flex; flex-direction: column; min-height: 100vh; }
        ...
        .card-hover { transition: transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer; border: 1px solid #dee2e6; }
        .card-hover:hover { transform: translateY(-4px); box-shadow: 0 12px 24px rgba(0,0,0,0.1) !important; border-color: #1e3a8a !important; }
        .card-hover:hover .book-cover { transform: scale(1.05); }
        ...
        <div class="col-lg-3">
            <div class="bg-white rounded-4 shadow-sm p-4 border">
                ...
                <?php if(!empty($latest_books)): ?>
                    ...
                    <?php $newBookJson = htmlspecialchars(json_encode($newBook), ENT_QUOTES, 'UTF-8'); ?>
                    <div class="d-flex align-items-center mb-3 pb-3 border-bottom card-hover p-2 rounded" onclick="openDetailsModal(<?= $newBookJson ?>)">
                        <?php if(!empty($newBook['cover_photo'])): ?>
                            <img src="<?= base_url('uploads/covers/' . esc($newBook['cover_photo'])) ?>" class="rounded shadow-sm me-3 book-cover" style="width: 50px; height: 70px; object-fit: cover;">
                        <?php else: ?>
                            <div class="bg-light rounded d-flex align-items-center justify-content-center me-3 shadow-sm text-secondary border book-cover" style="width: 50px; height: 70px;">
                                <i class="bi <?= $newBook['icon'] ?> fs-4"></i>
                            </div>
                        <?php endif; ?>
        ...
        <div class="row g-4">
            <div class="col-lg-5 col-md-6">
                <div class="d-flex align-items-center mb-3">
                    <img src="<?= base_url('images/ca_logo.png') ?>" alt="CA Logo" style="width: 55px;" class="me-3 bg-white rounded-circle p-1 shadow-sm">
                    <div>
