    ...
    function openDetailsModal(item, calculatedStatus) {
        ...
        // 1. Populate Text (Matched exactly to the HTML IDs in catalog.php)
        document.getElementById('modalTitle').innerText = item.title || 'No Title';
        ...
        // Safely set the Subject / Publisher
        document.getElementById('modalSubject').innerText = item.subject || item.publisher || 'N/A';

        // Safely format the date
        let dateStr = 'N/A';
        if (item.issued_date) {
            let d = new Date(item.issued_date);
            dateStr = isNaN(d.getTime()) ? item.issued_date : d.toLocaleDateString('en-US', { year: 'numeric', month: 'short' });
        }
        ...
        // 4. Background Tracking (Fixed 'book' to 'item')
        try {
            let formData = new FormData();
            formData.append('id', item.id);
            formData.append('type', item.type);
            fetch('<?= base_url('track-view') ?>', { method: 'POST', body: formData }).catch(() => {});
        } catch(e) { console.error("Tracking error", e); }

        // 5. Action Form (Request button logic)
        let actionHtml = '';
