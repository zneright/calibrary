                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <div class="btn-group shadow-sm">
                                        <button class="btn btn-sm btn-light border text-info view-btn" title="View Details" data-row="<?= $jsonData ?>">
                                            <i class="bi bi-eye"></i>
                                        </button>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-light border-0">
                <h5 class="modal-title fw-bold text-primary">Collection Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                <div class="row g-0">
                    <div class="col-md-5 bg-light d-flex flex-column align-items-center justify-content-center p-4 border-end" id="viewCoverContainer" style="min-height: 350px;">
                        </div>
                    
                    <div class="col-md-7 p-4">
                        <span id="viewStatusBadge" class="badge mb-2 px-3 py-2"></span>
                        
                        <h4 id="viewTitle" class="fw-bold text-dark mb-1"></h4>
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                <small class="text-muted d-block text-uppercase" style="font-size: 0.7rem;">Resource Type</small>
                                <span id="viewType" class="fw-bold text-dark"></span>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block text-uppercase" style="font-size: 0.7rem;">Accession No.</small>
                                <span id="viewAccession" class="fw-bold text-dark"></span>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block text-uppercase" style="font-size: 0.7rem;">Class / Call No.</small>
                                <span id="viewClass" class="text-dark"></span>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block text-uppercase" style="font-size: 0.7rem;">ISBN</small>
                                <span id="viewISBN" class="text-dark"></span>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block text-uppercase" style="font-size: 0.7rem;">Publisher</small>
                                <span id="viewPublisher" class="text-dark"></span>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block text-uppercase" style="font-size: 0.7rem;">Library Location</small>
                                <span id="viewLocation" class="text-dark"></span>
                            </div>
                        </div>

                        <div id="viewActionsContainer" class="mt-4 pt-3 border-top d-flex gap-2"></div>
                        ...
                        <div class="col-lg-6 border-end px-3">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input list="typeOptions" name="type" id="manage_type" class="form-control shadow-sm" placeholder="Select or type new">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold small text-secondary">Reading</label>
                                    <input list="readingOptions" name="reading" id="manage_reading" class="form-control shadow-sm" placeholder="Select or type new">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold small text-secondary">Class</label>
                                    <input list="classOptions" name="class" id="manage_class" class="form-control shadow-sm" placeholder="Select or type new class">
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-12">
                                    ...
                                    <input list="subjectOptions" name="subject" id="manage_subject" class="form-control shadow-sm" placeholder="Select or type new subject">
                                </div>
                                <div class="col-md-8">
                                    <label class="form-label fw-semibold small text-secondary">Author</label>
                                    <input list="authorOptions" name="author" id="manage_author" class="form-control shadow-sm" placeholder="Select or type new author">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold small text-secondary">Publisher</label>
                                    <input list="publisherOptions" name="publisher" id="manage_publisher" class="form-control shadow-sm" placeholder="Select or type new">
                                </div>
                                <div class="col-md-6">
...
<div class="modal fade" id="addCollectionModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-light border-bottom text-secondary">
                <h5 class="modal-title fw-bold"><i class="bi bi-journal-plus me-2 text-primary"></i>Add New Collection</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= base_url('admin/collections/store') ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="modal-body p-4 bg-white">
                    <p class="text-muted">Use the Add button from your original code here.</p>
                </div>
                <div class="modal-footer bg-light border-top">
                    <button type="button" class="btn btn-secondary shadow-sm px-4" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-primary shadow-sm px-4 fw-semibold"><i class="bi bi-save me-1"></i> Save Collection</button>
                </div>
            </form>
        </div>
    </div>
...
<datalist id="typeOptions"><?php $defaultTypes = ['Book', 'Journal', 'Presidential Decree', 'Executive Order']; foreach($defaultTypes as $dt) echo "<option value=\"$dt\">"; if(!empty($types)): foreach($types as $t): if(!in_array($t['type'], $defaultTypes)) echo '<option value="' . esc($t['type']) . '">'; endforeach; endif; ?></datalist>
<datalist id="readingOptions"><?php $defaultReadings = ['General', 'Restricted']; foreach($defaultReadings as $dr) echo "<option value=\"$dr\">"; if(!empty($readings)): foreach($readings as $r): if(!in_array($r['reading'], $defaultReadings)) echo '<option value="' . esc($r['reading']) . '">'; endforeach; endif; ?></datalist>
<datalist id="classOptions"><?php if(!empty($classes)): foreach($classes as $c): ?><option value="<?= esc($c['class']) ?>"><?php endforeach; endif; ?></datalist>
...
<datalist id="publisherOptions"><?php if(!empty($publishers)): foreach($publishers as $p): ?><option value="<?= esc($p['publisher']) ?>"><?php endforeach; endif; ?></datalist>
...
$(document).ready(function () {
    ...
    $('input[name="soft_copy"]').on('change', function(e) {
        ...
        if (file && file.type === "application/pdf") {
            ...
            if(confirm("New PDF selected. Would you like to review/preview it before saving?")) {
                viewPDF(fileURL);
            }
        }
    });
    ...
    // 1. DEDICATED VIEW MODAL LOGIC
    $(document).on('click', '.view-btn', function() {
        const rawData = $(this).attr('data-row');
        let book = JSON.parse(rawData);

        $('#viewTitle').text(book.title || 'No Title');
        $('#viewAuthor').text(book.author || 'Unknown Author');
        $('#viewType').text(book.type || 'N/A');
        $('#viewAccession').text(book.accession_no || 'N/A');
        $('#viewClass').text(book.class || 'N/A');
        $('#viewPublisher').text(book.publisher || 'N/A');
        $('#viewISBN').text(book.isbn || 'N/A');
        $('#viewLocation').text(book.location || 'N/A');

        // Status Badge
        let statusBadge = $('#viewStatusBadge');
        let status = (book.status || 'AVAILABLE').toUpperCase();
        statusBadge.text(status);
        if(status === 'AVAILABLE') {
            statusBadge.attr('class', 'badge bg-success bg-opacity-10 text-success border border-success mb-2 px-3 py-2');
        } else if(status === 'BORROWED') {
            statusBadge.attr('class', 'badge bg-warning bg-opacity-10 text-warning border border-warning mb-2 px-3 py-2');
        } else {
            statusBadge.attr('class', 'badge bg-danger bg-opacity-10 text-danger border border-danger mb-2 px-3 py-2');
        }

        // Cover Photo Injection
        ...
        if (book.cover_photo) {
            ...
            coverContainer.html(`<i class="bi bi-book text-secondary" style="font-size: 8rem;"></i><p class="text-muted mt-3 small">No Cover Photo</p>`);
        }

        // Actions Injection (PDF and URL)
        let actionsHtml = '';
        if (book.soft_copy) {
            let pdfUrl = "<?= base_url('uploads/softcopies/') ?>" + book.soft_copy;
            actionsHtml += `<button class="btn btn-outline-danger btn-sm fw-bold shadow-sm flex-fill" onclick="viewPDF('${pdfUrl}')"><i class="bi bi-file-earmark-pdf me-1"></i> Read PDF</button>`;
        }
        if (book.url) {
            actionsHtml += `<a href="${book.url}" target="_blank" class="btn btn-outline-info btn-sm fw-bold shadow-sm flex-fill"><i class="bi bi-link-45deg me-1"></i> Visit Link</a>`;
        }
        if(!actionsHtml) {
            actionsHtml = '<span class="text-muted small w-100 text-center">No digital files attached.</span>';
        }
        $('#viewActionsContainer').html(actionsHtml);

        $('#viewCollectionModal').modal('show');
    });

    // 2. DEDICATED EDIT MODAL LOGIC
    $(document).on('click', '.edit-btn', function() {
        ...
        $('#manage_status').val(rowData.status ? rowData.status.toUpperCase() : '');
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        $('#delete_collection_title').val($(this).attr('data-title'));
