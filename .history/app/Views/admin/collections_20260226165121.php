        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <form id="manageForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer bg-light border-top">
                    ...
                    <button type="submit" id="manageSubmitBtn" class="btn btn-warning shadow-sm px-4 fw-bold">Update Changes</button>
                </div>
            </form>
        </div>
    </div>
                    ...
                    <div class="row g-4">
                        <div class="col-lg-6 border-end px-3">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold small text-secondary">Publisher</label>
                                    <input list="publisherOptions" name="publisher" class="form-control shadow-sm" placeholder="Select or type new" value="<?= old('publisher') ?>">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold small text-secondary">Place</label>
...
<div class="modal fade" id="imagePreviewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-transparent border-0 shadow-none text-center">
            ...
            <div class="mt-2"><button type="button" class="btn btn-light rounded-pill px-4 fw-bold" data-bs-dismiss="modal">Close</button></div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body p-0">
                <iframe src="" id="pdfFrame" width="100%" height="750px" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<datalist id="typeOptions">
    <?php 
        $defaultTypes = ['Book', 'Journal', 'Presidential Decree', 'Executive Order'];
        foreach($defaultTypes as $dt) echo "<option value=\"$dt\">";
        if(!empty($types)): foreach($types as $t): 
            if(!in_array($t['type'], $defaultTypes)) echo '<option value="' . esc($t['type']) . '">';
        endforeach; endif; 
    ?>
</datalist>
...
<datalist id="readingOptions">
    <?php 
        $defaultReadings = ['General', 'Restricted'];
        foreach($defaultReadings as $dr) echo "<option value=\"$dr\">";
        if(!empty($readings)): foreach($readings as $r): 
            if(!in_array($r['reading'], $defaultReadings)) echo '<option value="' . esc($r['reading']) . '">';
        endforeach; endif; 
    ?>
</datalist>

<datalist id="classOptions">
    <?php if(!empty($classes)): foreach($classes as $c): ?><option value="<?= esc($c['class']) ?>"><?php endforeach; endif; ?>
</datalist>

<datalist id="subjectOptions">
    <?php if(!empty($subjects)): foreach($subjects as $s): ?><option value="<?= esc($s['subject']) ?>"><?php endforeach; endif; ?>
</datalist>

<datalist id="authorOptions">
    <?php if(!empty($authors)): foreach($authors as $a): ?><option value="<?= esc($a['author']) ?>"><?php endforeach; endif; ?>
</datalist>

<datalist id="publisherOptions">
    <?php if(!empty($publishers)): foreach($publishers as $p): ?><option value="<?= esc($p['publisher']) ?>"><?php endforeach; endif; ?>
</datalist>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.view-btn, .edit-btn', function() {
        ...
        // Ensure Status Dropdown correctly selects the value from DB
