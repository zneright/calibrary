                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($row['status'] == 'Pending'): ?>
                                    <form action="<?= base_url('admin/transactions/approve') ?>" method="POST" class="d-inline">
                                        <?= csrf_field() ?>
                                ...
                                <?php elseif($row['status'] == 'Approved'): ?>
                                    <form action="<?= base_url('admin/transactions/updateStatus') ?>" method="POST" class="d-inline">
                                        ...
                                        <button type="submit" class="btn btn-info btn-sm text-white shadow-sm px-2">
                                            <i class="bi bi-hand-index-thumb"></i> Mark as On Hand
                                        </button>
                                    </form>
...
$(document).ready(function () {
    $('#transactionsTable').DataTable({
        "autoWidth": false,
        "order": [], // This preserves the oldest-first order from the Controller
        "language": { "emptyTable": "No pending or active transactions found." }
    });
});
</script>
<?= $this->endSection() ?>