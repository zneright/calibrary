...
<div class="d-flex justify-content-between align-items-center mb-4">
        <ul class="nav nav-pills bg-white p-2 rounded shadow-sm border">
            <li class="nav-item">
                <a class="nav-link <?= !isset($_GET['status']) ? 'active' : '' ?>" href="<?= base_url('admin/transactions') ?>">
                    All Transactions
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= (isset($_GET['status']) && $_GET['status'] == 'Pending') ? 'active' : '' ?>" href="<?= base_url('admin/transactions?status=Pending') ?>">
                    Pending <span class="badge bg-warning text-dark ms-1 small rounded-pill"><?= $counts['pending'] ?></span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= (isset($_GET['status']) && $_GET['status'] == 'Approved') ? 'active' : '' ?>" href="<?= base_url('admin/transactions?status=Approved') ?>">
                    Ready for Pickup <span class="badge bg-primary text-white ms-1 small rounded-pill"><?= $counts['approved'] ?></span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= (isset($_GET['status']) && $_GET['status'] == 'Borrowed') ? 'active' : '' ?>" href="<?= base_url('admin/transactions?status=Borrowed') ?>">
                    On Hand <span class="badge bg-info text-dark ms-1 small rounded-pill"><?= $counts['borrowed'] ?></span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= (isset($_GET['status']) && $_GET['status'] == 'Returned') ? 'active' : '' ?>" href="<?= base_url('admin/transactions?status=Returned') ?>">
                    Returned
                </a>
            </li>
        </ul>
