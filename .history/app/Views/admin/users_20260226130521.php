    ...
    <div class="container-fluid">
                    ...
                    <table class="table table-bordered table-hover w-100" id="usersTable">
                        ...
                        <tbody class="align-middle text-center">
                            ...
                            <tr>
                                ...
                                <td class="text-start">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <small class="text-muted"><i class="bi bi-telephone"></i> <?= esc($row['contact'] ?? 'N/A') ?></small>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <button class="btn btn-warning btn-sm edit-user-btn" data-id="<?= $row['id'] ?>"><i class="bi bi-pencil"></i></button>
                                    <?php if($row['status'] == 'Active'): ?>
                                        <button class="btn btn-danger btn-sm deactivate-user-btn" data-id="<?= $row['id'] ?>" title="Deactivate Account"><i class="bi bi-person-x"></i></button>
                                    <?php else: ?>
                                        <button class="btn btn-success btn-sm" onclick="reactivateUser(<?= $row['id'] ?>)" title="Reactivate"><i class="bi bi-person-check"></i></button>
                                    <?php endif; ?>
                                </td>
                            </tr>
        ...
        <div class="modal fade" id="imageZoomModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content bg-transparent border-0 text-center">
                    ...
                    <div class="mt-3"><button class="btn btn-light rounded-pill px-4 fw-bold" data-bs-dismiss="modal">Close</button></div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="addUserModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form action="<?= base_url('admin/users/store') ?>" method="POST" class="modal-content border-0 shadow">
                    <?= csrf_field() ?>
                    <div class="modal-header"><h5>Add User</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-12"><label class="form-label fw-bold">Full Name</label><input type="text" name="fullname" class="form-control" required></div>
                            <div class="col-md-6"><label class="form-label fw-bold">User ID</label><input type="text" name="user_id" class="form-control" required></div>
                            <div class="col-md-6"><label class="form-label fw-bold">Contact No.</label><input type="text" name="contact" class="form-control" placeholder="09xxxxxxxxx"></div>
                            <div class="col-md-6"><label class="form-label fw-bold">Email</label><input type="email" name="email" class="form-control" required></div>
                            <div class="col-md-6"><label class="form-label fw-bold">Role</label><select name="role" class="form-select"><option value="Borrower">Borrower</option><option value="Admin">Admin</option></select></div>
                            <div class="col-md-6"><label class="form-label fw-bold">Password</label><input type="password" name="password" class="form-control" required></div>
                            <div class="col-md-6"><label class="form-label fw-bold">Re-enter Password</label><input type="password" name="confirm_password" class="form-control" required></div>
                        </div>
                    </div>
                    <div class="modal-footer"><button type="submit" class="btn btn-primary w-100">Create Account</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function () {
        $(document).ready(function () {
    // 1. Initialize Datatable - Search is automatic for all columns
        ...
        if(confirm('Are you sure you want to deactivate this user? They will not be deleted but will lose access.')) {
            let id = $(this).data('id');
            $('<form action="<?= base_url('admin/users/delete') ?>" method="POST">' +
        ...
        $('#usersTable').DataTable({
            "autoWidth": false,
            "order": [], // <-- Add this empty array here!
