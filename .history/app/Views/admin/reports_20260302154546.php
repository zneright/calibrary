                ...
                <div class="table-responsive p-3">
                    <table class="table table-hover table-bordered align-middle w-100 report-table">
                        <thead class="table-light text-muted small text-center text-uppercase">
                            <tr>
                                <th>Acc / Call No.</th>
                                <th class="text-start">Title & Subject</th>
                        ...
                        <tbody class="text-center small">
                            <?php if(!empty($collections)): foreach($collections as $row): ?>
                                <tr>
                                    <td>
                                        <strong><?= esc($row['accession_no'] ?: 'N/A') ?></strong><br>
                                        <small class="text-muted">Class: <?= esc($row['class'] ?: 'N/A') ?></small>
                                    </td>
                                    <td class="text-start">
                                        <span class="fw-bold"><?= esc($row['title']) ?></span><br>
                                        <small class="text-muted"><?= esc($row['subject']) ?></small>
                                    </td>
                                    <td>
                                        <?= esc($row['author']) ?><br>
                                        <small class="text-muted">Pub: <?= esc($row['publisher'] ?: 'N/A') ?></small>
                                    </td>
                                    <td>
                                        <small>Type: <?= esc($row['type']) ?></small><br>
                                        <small class="text-muted">Acq: <?= $row['date_acquired'] ? date('M d, Y', strtotime($row['date_acquired'])) : 'N/A' ?></small>
                                    </td>
                                    <td>
                                        <?php 
                                            $s = strtoupper($row['status']);
                                            $b = 'bg-secondary';
                                            if($s == 'AVAILABLE') $b = 'bg-success';
                                            if($s == 'BORROWED') $b = 'bg-warning text-dark';
                                            if($s == 'LOST' || $s == 'DAMAGED') $b = 'bg-danger';
                                        ?>
                            ...
                            <?php if(!empty($journals)): foreach($journals as $row): ?>
                                <tr>
                                    <td class="text-start">
                                        ...
                                        <small class="text-muted">Date: <?= $row['date'] ? date('M d, Y', strtotime($row['date'])) : 'N/A' ?></small>
                                    </td>
                                    <td><?= esc($row['author']) ?></td>
                                    <td>
                                        <?= esc($row['source']) ?><br>
                                        <small class="text-muted">Sess: <?= esc($row['session'] ?: 'N/A') ?></small>
                                    </td>
                                    <td>Vol <?= esc($row['volume'] ?: '-') ?>, Pg <?= esc($row['page'] ?: '-') ?></td>
                                    <td>
                                        <span class="badge bg-info text-dark"><?= esc($row['status'] ?? 'AVAILABLE') ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; endif; ?>
                        </tbody>
                    ...
                    <table class="table table-hover table-bordered align-middle w-100 report-table">
                        <thead class="table-light text-muted small text-center text-uppercase">
                            <tr>
                                ...
                                <th class="text-start">Item Requested</th>
                                <th>Dates</th>
                                <th>Status</th>
                            </tr>
                        </thead>
                        <tbody class="text-center small">
                            <?php if(!empty($transactions)): foreach($transactions as $row): ?>
                                <tr>
                                    <td class="fw-bold">TRX-<?= str_pad($row['id'], 5, '0', STR_PAD_LEFT) ?></td>
                                    <td class="text-start">
                                        <?= esc($row['user_name']) ?><br>
                                        <small class="text-muted">ID: <?= esc($row['user_id_num']) ?></small>
                                    </td>
                                    <td class="text-start">
                                        <strong><?= esc($row['collection_title']) ?></strong><br>
                                        <small class="text-muted">Reason: <?= esc($row['reason'] ?: 'N/A') ?></small>
                                    </td>
                                    <td>
                                        <small>Req: <?= date('M d, Y', strtotime($row['date_requested'])) ?></small><br>
                                        <small class="text-danger">Due: <?= $row['due_date'] ? date('M d, Y', strtotime($row['due_date'])) : '---' ?></small>
                                    </td>
                                    <td>
...
$(document).ready(function () {
    ...
    //checkbox states to form submit
