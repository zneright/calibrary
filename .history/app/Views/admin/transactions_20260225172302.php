...
<div class="container-fluid">
                        ...
                        <tr>
                            ...
                            <td>
                                <?php 
                                    ...
                                    if($row['status'] == 'Renewing') $badge = 'bg-warning text-dark'; // Match yellow for renewing
    ...
    <?php if($row['status'] == 'Pending'): ?>
        <div class="d-flex gap-1 justify-content-center">
            <form action="<?= base_url('admin/transactions/approve') ?>" method="POST">
                ...
                <button type="submit" class="btn btn-primary btn-sm shadow-sm" title="Approve">Approve</button>
            </form>
            <form action="<?= base_url('admin/transactions/reject') ?>" method="POST" onsubmit="return confirm('Reject this request?')">
                <?= csrf_field() ?>
                <input type="hidden" name="id" value="<?= $row['id'] ?>">
                <button type="submit" class="btn btn-outline-danger btn-sm shadow-sm" title="Reject"><i class="bi bi-x-lg"></i></button>
            </form>
        </div>
    ...
    <?php elseif($row['status'] == 'Approved'): ?>
        ...
        <div class="d-flex gap-1 justify-content-center">
            <button class="btn btn-sm btn-warning fw-bold shadow-sm admin-renew-btn" 
                    data-id="<?= $row['id'] ?>" 
                    data-title="<?= esc($row['collection_title']) ?>">
                Process Renewal
            </button>
            <form action="<?= base_url('admin/transactions/reject') ?>" method="POST" onsubmit="return confirm('Disapprove this extension?')">
                <?= csrf_field() ?>
                <input type="hidden" name="id" value="<?= $row['id'] ?>">
                <button type="submit" class="btn btn-outline-danger btn-sm shadow-sm"><i class="bi bi-x-lg"></i></button>
            </form>
        </div>
