                ...
                <table class="table table-bordered table-hover w-100" id="journalsTable">
                    ...
                    <tbody class="align-middle text-center">
                        ...
                        <tr>
                            ...
                            <td><?= $row['date'] ? date('M d, Y', strtotime($row['date'])) : '<span class="text-muted">N/A</span>' ?></td>
                            <td>
...
<datalist id="sourceOptions">
    <?php if(!empty($sources)): foreach($sources as $src): ?><option value="<?= esc($src['source']) ?>"><?php endforeach; endif; ?>
</datalist>

<datalist id="sessionOptions">
    <?php if(!empty($sessions)): foreach($sessions as $sess): ?><option value="<?= esc($sess['session']) ?>"><?php endforeach; endif; ?>
</datalist>

<datalist id="volumeOptions">
    <?php if(!empty($volumes)): foreach($volumes as $v): ?><option value="<?= esc($v['volume']) ?>"><?php endforeach; endif; ?>
</datalist>

<datalist id="pageOptions">
    <?php if(!empty($pages)): foreach($pages as $p): ?><option value="<?= esc($p['page']) ?>"><?php endforeach; endif; ?>
</datalist>
...
$(document).ready(function () {
    ...
    // OPEN ADD MODAL
    $('button[data-bs-target="#addCollectionModal"]').on('click', function() {
        $('#manage_id').val('');
        $('#manageForm')[0].reset(); // Clear all fields
