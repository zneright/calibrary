...
$(document).ready(function () {
    ...
    // --- 1. PDF VIEWER FUNCTION ---
    window.viewPDF = function(url) {
        $('#pdfFrame').attr('src', url);
        $('#pdfViewerModal').modal('show');
    };

    // --- 2. REVIEW PDF BEFORE SAVING ---
    // This script watches file inputs. When a file is chosen, it offers a preview.
    $('input[name="soft_copy"]').on('change', function(e) {
        const file = e.target.files[0];
        if (file && file.type === "application/pdf") {
            const fileURL = URL.createObjectURL(file);
            
            // Ask admin if they want to review
    ...
    // --- 3. AUTO URL BEHAVIOR ---
    // (Handled by target="_blank" in the HTML <a> tag added above)
