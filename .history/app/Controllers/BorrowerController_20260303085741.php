...
public function verifyResetView()
{
    ...
    return view('borrower/verify_reset', $data); 
}
...
public function processReset()
{
    ...
    // Identify the user via session or the temporary ID from the forgot password flow
    ...
    // Cleanup: Remove temp ID and destroy current session to force a fresh login
    session()->remove('temp_reset_user_id');
    if (session()->get('id')) {
        session()->destroy(); 
    }

    // REDIRECT TO LOGIN (The "Return to Log in" requirement)
    return redirect()->to('/login')->with('success', 'Password updated successfully! Please log in with your new credentials.');
}
