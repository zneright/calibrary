...
class ExportReport extends BaseController
{
    public function pdf()
    {
        ...
        $dompdf->setPaper('A4', 'landscape'); // Landscape gives us room for more details
        $dompdf->render();
    ...
    private function getFilteredData()
    {
        ...
        if (empty($categories)) {
            $categories = ['collections', 'journals', 'transactions']; // Default to all if no filter submitted
        }

        // 1. Collections Filter (Sorted Recent First)
        ...
        // 2. Journals Filter (Sorted Recent First)
        ...
        // 3. Transactions Filter (Sorted Recent First)
    ...
    private function generateHtml($data)
    {
        ...
        // --- COLLECTIONS TABLE WITH EXTRA INFO ---
        if ($data['showCollections'] && !empty($data['collections'])) {
            ...
            $html .= '<table>
                        <thead>
                            <tr>
                                <th width="12%">Accession / Call No.</th>
            ...
            foreach ($data['collections'] as $row) {
                $html .= '<tr>
                            ...
                            <td>Type: ' . esc($row['type']) . '<span class="sub-text">Loc: ' . esc($row['location'] ?: 'N/A') . '</span></td>
        ...
        // --- JOURNALS TABLE WITH EXTRA INFO ---
        ...
        // --- TRANSACTIONS TABLE WITH EXTRA INFO ---
        if ($data['showTransactions'] && !empty($data['transactions'])) {
            ...
            foreach ($data['transactions'] as $row) {
                ...
                $html .= '<tr>
                            ...
                            <td>Req: ' . date('M d, Y', strtotime($row['date_requested'])) . '<span class="sub-text">Due: ' . ($row['due_date'] ? date('M d, Y', strtotime($row['due_date'])) : '---') . '</span></td>
                            <td><strong>' . esc($row['status']) . '</strong></td>
                          </tr>';
            }
            $html .= '</tbody></table>';
