 ...
 public function index()
{
    ...
    // 1. Count Currently Borrowed (On Hand)
    $borrowedCount = $db->table('transactions')
        ->where(['user_id_num' => $userId, 'status' => 'Borrowed'])
        ->countAllResults();

    // 2. Count Pending (Waiting for Admin Approval)
    $pendingCount = $db->table('transactions')
        ->where(['user_id_num' => $userId, 'status' => 'Pending'])
        ->countAllResults();

    // 3. Count Ready for Pickup (Approved but not yet taken)
    ...
    // 5. Fetch Active Transactions for the Table
   ...
   public function catalog()
{
    ...
    // 2. Build Journals Query (Mapping journal columns to match collections schema)
    // subject -> title, author -> author, source -> subject
    ...
    // Journals are usually always available unless you have a specific column for it
    if (!empty($selectedTypes) && !in_array('Journal', $selectedTypes) && count($selectedTypes) > 0) {
        $jrQuery->where('1=0'); // Force empty if Journal is not in filter
    }
    ...
    // 3. Get existing requests to disable buttons
    ...
    $data = array_merge($this->getNotificationData(), [
        'items'            => $finalResults,
        'total_results'    => count($finalResults), // Dynamic count based on filters
        'search'           => $search,
  ...
  public function myBooks()
{
    ...
    $data = array_merge($this->getNotificationData(), [
        // 1. Items physically with the borrower OR ready to be picked up
        'active_borrows'   => $transactionModel->where('user_id_num', $userId)
                                               ->whereIn('status', ['Approved', 'Borrowed'])
                                               ->orderBy('id', 'DESC')->findAll(),
        
        // 2. Items waiting for Admin to say Yes/No
        'pending_requests' => $transactionModel->where('user_id_num', $userId)
                                               ->where('status', 'Pending')
                                               ->orderBy('id', 'DESC')->findAll(),
        
        // 3. Completed or Closed transactions
      // Use 'whereIn' with all possible "closed" statuses
    'history' => $transactionModel->where('user_id_num', $userId)
                                    ->whereIn('status', ['Returned', 'Rejected', 'Cancelled', 'returned', 'rejected', 'cancelled'])
                                    ->orderBy('id', 'DESC')
                                    ->findAll(),
        'today'            => $today
    ]);
