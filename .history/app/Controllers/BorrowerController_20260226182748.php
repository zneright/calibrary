...
class BorrowerController extends BaseController
{
    ...
    public function index()
    {
        ...
        // 1. UPDATED: Count Currently Borrowed OR Renewing as "On Hand"
        ...
        // 2. Count Pending (New Requests waiting for approval)
        $pendingCount = $db->table('transactions')
            ->where(['user_id_num' => $userId, 'status' => 'Pending'])
            ->countAllResults();

        // 3. Count Ready for Pickup (Approved but not yet handed over)
        $pickupCount = $db->table('transactions')
            ->where(['user_id_num' => $userId, 'status' => 'Approved'])
            ->countAllResults();

        // 4. Count Overdue (Only items physically borrowed)
    ...
    public function catalog()
    {
        ...
        // 4. FIX: Get EXACT transaction statuses for the current user to display correct buttons
        $transactionModel = new \App\Models\TransactionModel();
        $transactions = $transactionModel->where('user_id_num', $userId)
                            ->whereIn('status', ['Pending', 'Approved', 'Borrowed', 'Renewing'])
                            ->findAll();
                            
        $userTransactions = [];
        foreach ($transactions as $t) {
            // Map the collection ID to the status of their request
        ...
        $data = array_merge($this->getNotificationData(), [
            ...
            'userTransactions' => $userTransactions // Pass the detailed user transactions
        ]);
    ...
    public function submitRequest()
    {
        ...
        $sourceTable  = $this->request->getPost('source_table'); // Tells us if it's a 'collection' or 'journal'
        
        $item = null;
        $itemTitle = '';
        
        // 1. Check the correct table based on where the item came from
        if ($sourceTable === 'journals') {
            $item = $db->table('journals')->where('id', $collectionId)->get()->getRowArray();
            if ($item) {
                $itemTitle = $item['subject']; // Journals use 'subject' as the title
            }
        } else {
        ...
        // Validate if item exists and is Available or Damaged
        if (!$item || !in_array($itemStatus, ['AVAILABLE', 'DAMAGED'])) {
            return redirect()->back()->with('error', 'Sorry, this item is currently unavailable or borrowed by another user.');
        }

        // 2. Check if THIS user already has a pending request for this item
        $userId = session()->get('user_id');
        
        // (Optional: If you add an 'item_type' column to transactions later, check it here to prevent ID overlap)
        $existing = $transactionModel->where('user_id_num', $userId)
