  ...
  public function store()
{
    ...
    // Check if an ACTIVE user with this ID or Email exists
    $existing = $userModel->groupStart()
    ...
    if ($existing) {
        return redirect()->back()->with('error', 'User ID or Email is already registered and active.');
    }
...
public function delete()
{
    ...
    if ($user['role'] === 'Admin') {
        ...
        if ($adminCount <= 1) {
            return redirect()->back()->with('error', 'Cannot deactivate the only remaining Admin.');
        }
    }
    ...
    public function delete()
{
    ...
    if ($userModel->delete($id)) {
        ...
        $logModel->insert([
            ...
            'module'      => 'User Management',
            'action'      => 'Delete',
            'details'     => "Admin ($adminName) deleted user account for {$userToDelete['fullname']} ({$userToDelete['user_id']})."
        ]);

        $this->sendEmailNotification($userToDelete['email'], $userToDelete['fullname'], 'rejected');
        return redirect()->back()->with('success', 'User account deleted and notified via email.');
    }
