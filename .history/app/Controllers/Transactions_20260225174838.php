...
public function reject()
{
    ...
    if ($trans) {
        if ($trans['status'] === 'Renewing') {
            ...
            // We revert to 'Borrowed' so the action button becomes "Return" again
            ...
            $body = "Your request to extend '{$trans['collection_title']}' was declined. Please return the book by the original due date.";
        } else {
            // Case: New Borrow Request Denied
            $transactionModel->update($id, ['status' => 'Rejected']);
            // Make book available in catalog again
