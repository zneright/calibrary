...
class BorrowerController extends BaseController
{
    ...
    private function getNotificationData()
    {
        ...
        // If no user is logged in, return empty data to prevent the TypeError
        if (!$userId) {
            return ['notifs' => [], 'unreadCount' => 0];
        }

        // Cast to string to be safe for CI4 strict typing
        $userId = (string) $userId;

        $notifs = $notificationModel->groupStart()
                ->where('target_audience', 'all_users')
                ->orLike('recipient', $userId) // Now guaranteed to be a string
            ->groupEnd()
    ...
    public function verifyResetView()
{
    ...
    // Only fetch notification data if the user is actually logged in
    // to avoid the "orLike() null" error
    ...
    public function processReset()
    {
        ...
        // 1. Identify the user (either logged in OR via the temp session from forgot password)
        ...
        // 3. ACTUAL DB UPDATE - Using $id to ensure the right record is updated
        $updateData = [
    ...
    {
        // 1. Explicitly call the model to avoid namespace errors
        $userModel = new \App\Models\UserModel();
        ...
        // 2. Check if code matches OR if the time has expired
        if ($user['reset_token'] !== $inputCode || strtotime(date('Y-m-d H:i:s')) > strtotime($user['reset_expires'])) {
            return $this->response->setJSON([
                'success' => false, 
    ...
    public function cancelReset()
    {
        ...
        // Find who is trying to reset (logged in OR guest)
        $id = session()->get('id') ?? session()->get('temp_reset_user_id');

        if ($id) {
            // Invalidate the token in the database for security
            $userModel->update($id, [
                'reset_token'   => null,
                'reset_expires' => null
            ]);
        ...
        // Redirect back to the correct page with a message
        if (session()->get('id')) {
        ...
        // For guests (Forgot Password flow)
