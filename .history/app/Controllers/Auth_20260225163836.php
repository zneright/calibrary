...
class Auth extends BaseController
{
    ...
    {
        $session = session();
        $session->destroy(); // Wipes all data for this specific browser
  ...
  public function attemptLogin()
    ...
    // 1. SECURITY: Destroy any old session data left in memory
    $session->destroy(); 
    
    // 2. Start a fresh session instance
    $session = session(); 
    
    // 3. GENERATE A NEW SESSION ID (Prevents overwriting)
    $session->regenerate(); 
