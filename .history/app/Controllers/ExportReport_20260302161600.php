...
class ExportReport extends BaseController
{
   ...
   public function pdf()
    ...
    private function generateHtml($data)
    {
        ...
        <head>
            <style>
                /* Setup page margins to prevent content overlapping header/footer */
                @page { margin: 110px 25px 50px 25px; }

                /* Fixed Header Setup */
                header { 
                    position: fixed; top: -90px; left: 0px; right: 0px; height: 90px; 
                    border-bottom: 2px solid #1e3a8a; text-align: center;
                }

                /* Fixed Footer Setup */
                footer { 
                    position: fixed; bottom: -30px; left: 0px; right: 0px; height: 30px; 
