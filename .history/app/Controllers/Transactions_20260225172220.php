...
public function reject()
{
    ...
    if ($trans) {
        ...
        $this->logAction("Rejected request/renewal for '{$trans['collection_title']}' by {$trans['user_name']}.");

        // Optional: Send email to user informing them of the rejection
        $userModel = new \App\Models\UserModel();
        $user = $userModel->where('user_id', $trans['user_id_num'])->first();
        if ($user && !empty($user['email'])) {
            $subject = 'Update on your Resource Request - CALIS v2.0';
            $message = "
                <div style='font-family: Arial, sans-serif; padding: 20px; border: 1px solid #dee2e6; border-radius: 8px;'>
                    <h3 style='color: #dc3545;'>Request Declined</h3>
                    <p>Hello {$trans['user_name']},</p>
                    <p>We regret to inform you that your request for <b>\"{$trans['collection_title']}\"</b> has been declined by the administrator.</p>
                    <p>If you have questions, please visit the library desk.</p>
                </div>";
