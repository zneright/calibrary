  ...
  public function attemptLogin()
{
    ...
    // --- SECURITY: Clear any old session before starting a new one ---
    $session->destroy();
    $session = session(); // Re-initialize fresh session

    $userIdInput = trim($this->request->getPost('username'));
    $passwordInput = $this->request->getPost('password');

    $user = $userModel->where('user_id', $userIdInput)->orWhere('email', $userIdInput)->first();

    if ($user && password_verify($passwordInput, $user['password'])) {
        if ($user['is_verified'] == 0) {
            return redirect()->to('/login')->with('error', 'Pending admin approval.');
        }

        // --- SESSION DATA: Store specific info for the current user ---
        $session->set([
            'id'         => $user['id'],
            'user_id'    => $user['user_id'],
            'fullname'   => $user['fullname'], // This ensures the specific name shows
            'role'       => $user['role'],
            'avatar'     => $user['avatar'],
            'isLoggedIn' => TRUE
        ]);
