...
public function reject()
{
    ...
    if ($trans) {
        if ($trans['status'] === 'Renewing') {
            // IF EXTENSION DENIED: Revert to 'Borrowed' so they can still 'Return' it later
            $transactionModel->update($id, ['status' => 'Borrowed']);
            $successMsg = "Extension denied. Item remains On Hand.";
            
            // Notify the user via email
            $subject = "Extension Request Denied";
            $message = "Your extension request for '{$trans['collection_title']}' was denied. Please return it by your original due date.";
            $this->sendEmail($this->getUserEmail($trans['user_id_num']), $subject, $message);
        } else {
            // IF INITIAL REQUEST DENIED: Mark as Rejected and make book available
            $transactionModel->update($id, ['status' => 'Rejected']);
            $colModel = new \App\Models\CollectionModel();
            $colModel->update($trans['collection_id'], ['status' => 'AVAILABLE']);
            $successMsg = "Request rejected. Book is now available in catalog.";
        }
