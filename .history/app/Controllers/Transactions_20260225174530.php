...
public function reject()
{
    ...
    if ($trans) {
        $userEmail = $this->getUserEmail($trans['user_id_num']);

        if ($trans['status'] === 'Renewing') {
            // 1. EXTENSION DENIED: Revert to 'Borrowed' so it stays on hand
            $transactionModel->update($id, ['status' => 'Borrowed']);
            
            $subject = 'Extension Request Denied';
            $body = "Your request to extend '{$trans['collection_title']}' was declined. Please return it by: " . date('M d, Y', strtotime($trans['due_date']));
            $this->sendEmail($userEmail, $subject, $body);
            
            $msg = "Extension denied for '{$trans['collection_title']}'. Item remains On Hand.";
        } else {
            // 2. NEW REQUEST DENIED: Mark as Rejected and make book available
