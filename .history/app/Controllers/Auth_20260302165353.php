...
public function sendResetLink() {
    ...
    try {
        ...
        $mail->Body = "
            ...
            <p>You requested a password reset for your CALIS account.</p>
            <p>Click the link below to set a new password. This link expires in 1 hour.</p>
            <a href='$resetLink' style='background:#1a2942; color:white; padding:10px 20px; text-decoration:none; border-radius:5px;'>Reset Password</a>
        ";
        $mail->send();
...
public function resetPassword($token) {
    ...
    if (!$user) {
        return redirect()->to('/login')->with('error', 'The reset link is invalid or has expired.');
    }
...
public function updateResetPassword() {
    ...
    if ($user) {
        $userModel->update($user['id'], [
            ...
            'reset_token'   => null, // Clear token after success
            'reset_expires' => null
        ]);
        return redirect()->to('/login')->with('success', 'Password reset successful! You can now log in.');
    }
