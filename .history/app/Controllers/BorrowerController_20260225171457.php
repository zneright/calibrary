 ...
 public function index()
    {
        ...
        // 5. UPDATED: Fetch Active Transactions including 'Renewing' for the dashboard table
        $activeBorrows = $db->table('transactions')
    ->select('transactions.*, collections.class') // Pulls all trans data + collection class
    ->join('collections', 'collections.id = transactions.collection_id', 'left')
    ->where('transactions.user_id_num', $userId)
    // Ensure 'Renewing' is in this list so it appears in the dashboard table
