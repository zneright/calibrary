...
class ExportReport extends BaseController
{
    ...
    public function excel()
    {
        ...
        // Build the HTML structure (Excel can read HTML tables natively)
        $output = "<html><meta charset='utf-8'><body>";
        $output .= "<table border='1'>";
        $output .= "<tr><th colspan='6' style='font-size: 16px; font-weight: bold;'>CALIS Library System - Generated Report</th></tr>";
        $output .= "<tr><th colspan='6'>Date Generated: " . date('F d, Y h:i A') . "</th></tr>";
        $output .= "<tr><th colspan='6'></th></tr>"; // Empty row for spacing

        // Collections Data
        if ($data['showCollections'] && !empty($data['collections'])) {
            $output .= "<tr><th colspan='6' style='background-color: #d9e1f2; font-weight: bold;'>Collections Inventory Report</th></tr>";
            $output .= "<tr style='background-color: #1e3a8a; color: #ffffff;'>
            ...
            $output .= "<tr><th colspan='6'></th></tr>"; // Empty row
        }

        // Journals Data
        if ($data['showJournals'] && !empty($data['journals'])) {
            $output .= "<tr><th colspan='6' style='background-color: #d9e1f2; font-weight: bold;'>Journals Directory Report</th></tr>";
            $output .= "<tr style='background-color: #17a2b8; color: #ffffff;'>
        ...
        if ($data['showTransactions'] && !empty($data['transactions'])) {
            $output .= "<tr><th colspan='6' style='background-color: #d9e1f2; font-weight: bold;'>Library Transactions Report</th></tr>";
            $output .= "<tr style='background-color: #0f766e; color: #ffffff;'>
        ...
        $filename = 'CALIS_Library_Report_' . date('Ymd') . '.xls';

        // Use CodeIgniter 4's native download response (This prevents file corruption)
        return $this->response->download($filename, $output)->setContentType('application/vnd.ms-excel');
    }
