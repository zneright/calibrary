    ...
    public function verifyResetView()
{
    ...
    public function processReset()
    {
        ...
        // Find user (logged in OR via the temp session from forgot password)
        ...
        if ($userModel->update($id, $updateData)) {
            ...
            $logModel = new LogModel();
            $logModel->insert([
                'user_name'   => $user['fullname'], // Use the DB name in case they are a guest
                'user_id_num' => $user['id_num'] ?? $id, // Use their ID number, fallback to DB ID
                'module'      => 'User Profile', 
            ...
            // If they were logged in while doing this, destroy their session so they have to log in with the new password
            if (session()->get('id')) {
                session()->destroy(); 
            }
