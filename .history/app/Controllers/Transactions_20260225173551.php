...
public function reject()
{
    ...
    if ($trans) {
        if ($trans['status'] === 'Renewing') {
            // Extension Denied: Put back to Borrowed so they can still 'Return' it
            $transactionModel->update($id, ['status' => 'Borrowed']);
            $msg = "Extension for '{$trans['collection_title']}' was denied. Item remains On Hand.";
            
            // Notify user of denial
            $subject = 'Extension Request Denied';
            $body = "Your request to extend '{$trans['collection_title']}' was declined. Please return it by the original date: " . date('M d, Y', strtotime($trans['due_date']));
