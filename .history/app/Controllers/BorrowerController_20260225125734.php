  ...
  public function index()
{
    ...
    // 1. Count Currently Borrowed (Status is 'Borrowed')
    $borrowedCount = $db->table('transactions')
        ->where('user_id_num', $userId)
        ->where('status', 'Borrowed')
        ->countAllResults();

    // 2. Count Pending Requests (Status is 'Pending')
    $pendingCount = $db->table('transactions')
        ->where('user_id_num', $userId)
        ->where('status', 'Pending')
        ->countAllResults();

    // 3. Count Overdue Items (Status is 'Borrowed' AND due_date < today)
    // Note: Ensure your transactions table has a 'due_date' column
    ...
    // 4. Fetch Active Borrows for the Table
    // We join with the collections table to get the Call Number (class)
    ...
    // Combine with notification data
