        ...
        public function logout()
{
    ...
    public function attemptLogin()
    {
        ...
        if ($user && password_verify($passwordInput, $user['password'])) {
            ...
            // Security Check: Is the account approved?
            if ($user['is_verified'] == 0) {
                // Optional: Log that a pending user tried to log in
                $logModel = new \App\Models\LogModel();
                $logModel->insert([
                    ...
                    'module'      => 'Authentication',
                    'action'      => 'Failed',
                    'details'     => "Pending user ({$user['fullname']}) attempted to log in before admin approval."
                ]);
            ...
            if ($user['role'] === 'Admin') {
                return redirect()->to('/admin/admindashboard')->with('success', 'Welcome back, ' . $user['fullname'] . '!');
            } else {
                return redirect()->to('/borrower/admindashboard')->with('success', 'Welcome to the CALIS Library Portal!');
            }
