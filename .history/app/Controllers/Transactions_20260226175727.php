...
use App\Models\NotificationModel; // Added NotificationModel
...
class Transactions extends BaseController {
    ...
    // --- NEW HELPER: Send In-App Notification ---
    private function sendNotification($userId, $type, $message) {
        $notifModel = new NotificationModel();
        $notifModel->insert([
            'target_audience' => 'user', 
            'recipient'       => $userId,
            'type'            => $type, // e.g., 'success', 'warning', 'danger', 'info'
            'message'         => $message,
            'status'          => 'unread'
        ]);
    }
    ...
    public function approveRenewal()
    {
        ...
        $trans = $transactionModel->find($id); // Fetch to get user details
    ...
    public function approve() {
        ...
        if ($trans) {
            ...
            // 1. Approve the selected transaction
    ...
    public function processReturn() {
        ...
        if($trans) {
            ...
            // Update the Book Status in Collections Table back to Available
    ...
    public function reportIssue() {
        ...
        $id = $this->request->getPost('id');
        $issueType = $this->request->getPost('issue_type'); // 'Lost' or 'Damaged'
