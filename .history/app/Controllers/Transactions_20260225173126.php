...
public function reject()
{
    ...
    if ($trans) {
        // If it was a renewal, mark as Extension Rejected
        // If it was a first-time request, mark as Rejected
        $finalStatus = ($trans['status'] === 'Renewing') ? 'Extension Rejected' : 'Rejected';
        
        $transactionModel->update($id, ['status' => $finalStatus]);
        
        // If it was a first-time request, make the book available in the catalog again
        if ($finalStatus === 'Rejected') {
            $colModel = new \App\Models\CollectionModel();
            $colModel->update($trans['collection_id'], ['status' => 'AVAILABLE']);
        }

        return redirect()->back()->with('success', 'The request has been declined.');
    }
    return redirect()->back()->with('error', 'Transaction not found.');
}
}