...
public function reject()
{
    ...
    if ($trans) {
        if ($trans['status'] === 'Renewing') {
            // Logic: Extension Denied. 
            // Revert back to 'Borrowed' so it stays "Active" and "On Hand"
            $transactionModel->update($id, ['status' => 'Borrowed']);
            
            $msg = "Extension request for '{$trans['collection_title']}' was denied. Original due date stands.";
            $subject = "Book Extension Request Denied";
            $body = "Your request to extend <b>\"{$trans['collection_title']}\"</b> has been declined. Please return the book by the original due date: " . date('M d, Y', strtotime($trans['due_date']));
        } else {
            // Logic: New Borrow Request Denied. 
            // Set to 'Rejected' (Moves to History) and make book available again.
            $transactionModel->update($id, ['status' => 'Rejected']);
            $colModel = new \App\Models\CollectionModel();
            $colModel->update($trans['collection_id'], ['status' => 'AVAILABLE']);
            
            $msg = "Borrow request for '{$trans['collection_title']}' was rejected.";
            $subject = "Resource Request Declined";
            $body = "We regret to inform you that your request for <b>\"{$trans['collection_title']}\"</b> has been declined.";
        }

        $this->logAction($msg);
        $this->sendEmail($this->getUserEmail($trans['user_id_num']), $subject, $body);

        return redirect()->back()->with('success', 'Action processed and borrower notified.');
    }
    return redirect()->back()->with('error', 'Transaction not found.');
}
