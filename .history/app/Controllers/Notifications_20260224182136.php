    ...
    public function markRead()
{
    if ($this->request->isAJAX()) {
        ...
        $notificationModel->where('target_audience', 'all_admins')
                          ->where('status', 'unread')
                          ->set(['status' => 'read'])
                          ->update();
...
{
    // The $notifs data is already handled globally by BaseController
    // This method simply points to the new view
    ...
    $model->where('target_audience', 'all_admins')
          ->set(['status' => 'read'])
          ->update();
    return $this->response->setJSON(['status' => 'success']);
}
